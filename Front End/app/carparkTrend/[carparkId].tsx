...
export default function CarparkTrend() {
  ...
  useEffect(() => {
    ...
    const getForecast = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://192.168.68.60:3000/getAvailabilityForecast/${carparkId}`, {
          method: 'POST',
        });
  ...
  useEffect(() => {
    ...
    const getAvailabilityHistory = async () => {
      ...
      setGraphData([]);
      try {
        const response = await fetch(`http://192.168.68.60:3000/fetchCarparkHistory/${carparkId}/${startTime / 1000}/${endTime / 1000}`);
