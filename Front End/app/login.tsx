...
export default function LoginForm() {
  ...
  const handleLogin = async () => {
    ...
    try {
      let response
      if (isEmail()) response = await fetch(`http://10.130.2.118:3000/fetchbyEmail/${username}`);
      else response = await fetch(`http://10.130.2.118:3000/fetchbyUsername/${username}`);
  ...
  const setUserData = async (data: userDataIncomplete) => {
    console.log(data)
    try {
      let response = await fetch(`http://10.130.2.118:3000/fetchUserData/${username}`)
